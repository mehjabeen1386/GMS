...
const createOrderSchema = z.object({
  ...
  consumptionPerPieceMeters: z.coerce
    .number()
    .min(0.1, "Consumption is required"),
...
// Order Card Component with Soft Delete Support
function OrderCard({
  order,
  onOrderDeleted,
}: {
  ...
  // Handle soft delete when cross icon is clicked
  const handleSoftDelete = async (
    e: React.MouseEvent,
    orderId: string,
    orderNumber: string,
  ) => {
    e.stopPropagation(); // Prevents triggering card click/details view

    if (
      window.confirm(
        `Are you sure you want to move order ${orderNumber} to trash?`,
      )
    ) {
      try {
        await api.patch(`/orders/${orderId}/soft-delete`);
        onOrderDeleted(); // Refresh the list to remove the card from the dashboard
      } catch (err) {
  ...
  return (
    <div className="relative flex flex-col justify-between rounded-xl border border-border bg-card p-5 shadow-sm transition-all hover:border-primary/50">
      {/* ❌ TOP-RIGHT CROSS DELETE BUTTON */}
      <button
        onClick={(e) => handleSoftDelete(e, order.id, order.orderNumber)}
        className="absolute right-3 top-3 rounded-full p-1 text-muted-foreground hover:bg-rose-500/10 hover:text-rose-600 transition-colors z-10"
        title="Move to Trash (3-month recovery)"
      >
        <X className="h-4 w-4" />
      </button>

      {/* Card Content Wrapper (pr-6 prevents text from hiding behind the cross icon) */}
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const formattedOrders: JobOrder[] = list.map((item: any) => ({
        ...
        clientName:
          item.clientName || item.client || item.companyName || "Mohammad",
        styleName:
          item.styleName ||
          item.garmentType ||
          item.style ||
          ...
          "Men formal shirt",
        quantity:
          item.quantity || item.totalQuantity || item.targetQuantity || 100,
      ...
      <div className="flex flex-col justify-between gap-4 rounded-xl border border-border bg-card p-6 shadow-sm sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Track garment production orders, target quantities, and fulfillment
            progress.
          </p>
        </div>
        {
          <div className="flex items-center space-x-3">
            ...
            <Link
              href="/orders/trash"
              className="inline-flex items-center rounded-lg border border-border px-4 py-2 text-sm font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
              title="View Trashed Orders"
            >
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  {errors.consumptionPerPieceMeters && (
                    <p className="mt-1 text-xs text-destructive">
                      {errors.consumptionPerPieceMeters.message}
                    </p>
                  )}
                </div>
              </div>
