...
const createOrderSchema = z.object({
  ...
  styleName: z.string().min(2, 'Garment style name is required'),
  quantity: z.coerce.number().min(1, 'Quantity must be at least 1 piece'),
...
export default function JobOrdersPage() {
  ...
  const {
    ...
    defaultValues: {
      orderNumber: `JO-${new Date().getFullYear()}-${Math.floor(100 + Math.random() * 900)}`,
      clientName: '',
      styleName: '',
      quantity: 100,
    ...
    try {
      ...
      setOrders([
        ...
        },
        {
          ...
          clientName: 'FabIndia Exports',
          styleName: 'Women Printed Kurti',
          quantity: 1200,
        ...
        {
          id: 'ord-103',
          orderNumber: 'JO-2026-893',
          clientName: 'Zara Sourcing',
          styleName: 'Slim Fit Chino Trousers',
          quantity: 400,
  ...
  const onCreateOrder = async (data: CreateOrderFormData) => {
    ...
    try {
      ...
      setServerError(
        err.response?.data?.message || 'Failed to create job order. Please check inputs.'
      );
    }
  };
          ...
          <div className="w-full max-w-lg space-y-5 rounded-xl border border-border bg-card p-6 shadow-xl animate-in fade-in zoom-in-95">
            ...
            <form onSubmit={handleSubmit(onCreateOrder)} className="space-y-4">
              ...
              <div className="flex items-center justify-end space-x-3 border-t border-border pt-4">
                ...
                >
                  {isSubmitting ? 'Provisioning...' : 'Create Order'}
                </button>
              </div>
            </form>
          </div>
